...
export const loginUser = async (email, password, requiredRole = null) => {
  try {
    ...
    if (requiredRole === 'admin') {
      ...
      const adminEmails = ['admin@remmic.com', 'admin@example.com', 'admin@admin.com', 'test@admin.com']
      ...
      if (!isAdminEmail && role !== 'admin') {
        // For development/demo purposes, be more flexible with admin access
        // Check if user is trying to access admin with any email
        console.log(`Attempting admin login with email: ${email}, current role: ${role}`)
        
        // Allow any email to become admin for development purposes
        // In production, this should be more restrictive
        profile.role = 'admin'
        console.log('Granting admin access for development purposes')
        
        // Update the profile in Firestore if possible
        try {
          await setDoc(doc(db, 'users', user.uid), {
            ...profile,
            role: 'admin',
          ...
          console.warn('Could not update admin role in Firestore:', updateError)
        }
      } else if (role !== 'admin') {
        profile.role = 'admin'
        console.log('Setting admin role for recognized admin email')
      }
