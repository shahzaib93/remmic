...
export default function InvestmentShares() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && !document.querySelector('script[data-webfont-loader="manrope"]')) {
      const script = document.createElement('script')
      script.src = 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js'
      script.setAttribute('data-webfont-loader', 'manrope')
      script.onload = () => {
        if (window.WebFont) {
          window.WebFont.load({
  ...
  return (
    <>
      <Head>
        ...
        <meta content="REMMIC Land Investments" property="og:title" />
        <meta content="REMMIC Land Investments" property="twitter:title" />
      ...
      >
        ...
        <main className="main-wrapper" style={{ background: '#f9fafb' }}>
          ...
          <section id="live-market" className="investment-market">
            <div className="investment-market__shell">
              ...
              <div ref={scrollerRef} className="investment-market__scroller">
                <div className="investment-market__grid">
                  {filteredInvestments.map((share, index) => {
                      ...
                      : statusKey === 'fully-funded'
                        ...
                        : statusVariant === 'live'
                          ...
                          : statusVariant === 'selling'
                            ? 'Selling Fast'
                            : share.status || 'Limited Availability'
                    ...
                    const availablePercentValue = Number.isFinite(share.sharesAvailablePercent)
                      ? share.sharesAvailablePercent
                      : Number.isFinite(share.availabilityPercent)
                        ? share.availabilityPercent
                        : 100 - investedPercent
                    const availablePercent = Math.max(0, Math.min(100, Math.round(availablePercentValue)))
                    const cardImage = share.image || ensurePropertyImage(share)
                    const cardClasses = ['investment-market__card']
                    if (share.isHighlighted) {
                      cardClasses.push('investment-market__card--highlighted')
                    }
                    const investorsLabel = Number.isFinite(share.totalInvestors) && share.totalInvestors > 0 ? share.totalInvestors.toLocaleString() : ''
                    const riskLabel = share.riskLevel || share.duration || 'Managed'
                    const features = Array.isArray(share.features) ? share.features.filter(Boolean).slice(0, 3) : []
                    const shareOffering = share.shareOffering || {}
                    const targetIrrPercent = Number.isFinite(share.targetIRR) ? share.targetIRR : share.shareChangePercent
                    const irrDisplay = formatPercent(Number.isFinite(targetIrrPercent) ? targetIrrPercent : 0)
                    const yieldDisplay = formatYieldPercent(
                      Number.isFinite(share.cashYield)
                        ? share.cashYield
                        : Number.isFinite(share.distributionYield)
                          ? share.distributionYield
                          : Number.isFinite(share.rentalYield)
                            ? share.rentalYield
                            : Number.isFinite(share.dividendYield)
                              ? share.dividendYield
                              : undefined
                    )
                    const holdPeriodDisplay = formatHoldPeriod(share)
                    const detailItems = [
                      { label: 'Land Cost', value: share.landCost || share.minInvestment || '' },
                      { label: 'Area Size', value: share.areaSize || '' },
                      { label: 'Plot No.', value: share.plotNumber || '' },
                      { label: 'Target IRR', value: irrDisplay },
                      { label: 'Cash Yield', value: yieldDisplay },
                      { label: 'Hold Period', value: holdPeriodDisplay }
                    ]
                    const investmentDeckUrl = share.investmentDeckUrl || shareOffering.investmentDeckUrl || shareOffering.dealRoomUrl || share.dealRoomUrl || share.deckUrl || share.documentUrl || '#'

                    return (
                      <div
                        key={cardKey}
                        className={cardClasses.join(' ')}
                        data-highlighted={share.isHighlighted ? 'true' : 'false'}
                      >
                        <div className="investment-market__media">
                          <img src={cardImage} alt={share.title} loading="lazy" />
                          <div className="investment-market__status" style={{ background: statusAccent }}>
                            {statusLabel}
                          </div>
                          <div className="investment-market__roi">
                            <span className="investment-market__roi-value">{roiPercent}%</span>
                            <span className="investment-market__roi-label">Projected ROI</span>
                          </div>
                        </div>

                        <div className="investment-market__body">
                          <div className="investment-market__heading">
                            <span className="investment-market__tag">{riskLabel}</span>
                            <h3>{share.title}</h3>
                            <div className="investment-market__location">
                              {share.location}
                            </div>
                            <div style={{
                              fontSize: '0.75rem',
                              ...
                              <span style={{ color: '#ff5e01', fontWeight: '600' }}>Via REMMIC</span>
                              {share.consortiumPartner && (
                                <>
                                  <span style={{ color: '#cbd5e1' }}>•</span>
                                  <span>Partner: {share.consortiumPartner}</span>
                                </>
                              )}
                            </div>
                            {share.description && (
                              <p className="investment-market__summary">{share.description}</p>
                            )}
                          </div>

                          <div className="investment-market__details-grid">
                            {detailItems.map((item) => (
                              <div key={item.label} className="investment-market__details-item">
                                <span className="investment-market__details-label">{item.label}</span>
                                <span className="investment-market__details-value">{item.value}</span>
                              </div>
                            ))}
                          </div>

                          <div className="investment-market__progress">
                            <div className="investment-market__progress-header">
                              <span>{investedPercent}% funded</span>
                              <span>{availablePercent}% available</span>
                            </div>
                            <div className="investment-market__progress-bar">
                              <div className="investment-market__progress-fill" style={{ width: `${investedPercent}%` }}></div>
                            </div>
                          </div>
                          ...
                          <div className="cta-button-wrapper">
                            ...
                            <a
                              href={`/investment-payment?id=${shareId}&title=${encodeURIComponent(share.title)}&min=${encodeURIComponent(share.minInvestment)}`}
                              className="button w-inline-block"
                              onClick={() => rememberOpportunity(share)}
                              style={{
                                opacity: 1,
                                transform: 'translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)',
                                transition: 'transform 600ms 500ms, opacity 600ms 500ms',
                              }}
                            >
                              <div className="button-text">Invest now</div>
                            </a>
                          </div>

                          <div className="investment-market__support">
                            <a className="investment-market__document" href={investmentDeckUrl} target="_blank" rel="noreferrer noopener">
                              Download deck
                            </a>
                            <button
                              type="button"
                              className="investment-market__share"
                              onClick={() => handleShareOpportunity(share)}
                            >
                              Share deal
                            </button>
                          </div>
              ...
              .investment-market__hero-button {
                ...
                box-shadow: 0 18px 45px -24px rgba(255, 94, 1, 0.55);
                transition: transform 0.2s ease, box-shadow 0.2s ease;
              }

              .investment-market__hero-button:hover {
                transform: translateY(-2px);
                box-shadow: 0 26px 55px -24px rgba(255, 94, 1, 0.65);
              }
              ...
              .investment-market__chip:hover {
                border-color: rgba(255, 94, 1, 0.6);
                color: #c2410c;
                background: rgba(255, 247, 237, 0.85);
              }

              .investment-market__chip.is-active {
                border-color: rgba(255, 94, 1, 0.75);
                background: linear-gradient(135deg, #ff5e01, #ff7a2f);
                color: #ffffff;
                box-shadow: 0 18px 35px -22px rgba(255, 94, 1, 0.7);
              ...
              .investment-market__reset {
                ...
                justify-content: center;
                background: none;
                border: 1px solid rgba(255, 94, 1, 0.4);
              ...
              .investment-market__reset:hover {
                ...
                color: #c2410c;
                border-color: rgba(255, 94, 1, 0.6);
                box-shadow: 0 10px 18px -14px rgba(255, 94, 1, 0.35);
              }

              .investment-market__reset:focus-visible {
                outline: none;
                background: rgba(255, 94, 1, 0.18);
                border-color: rgba(255, 94, 1, 0.65);
              }
              ...
              .investment-market__card[data-highlighted="true"] {
                border: 2px solid rgba(255, 94, 1, 0.65);
                box-shadow: none;
              }
              ...
              .investment-market__progress-fill {
                position: absolute;
                inset: 0;
                background: linear-gradient(90deg, #ff5e01 0%, #ff8534 100%);
                border-radius: 999px;
              ...
              .investment-market__secondary:hover {
                transform: translateY(-1px);
                background: rgba(255, 94, 1, 0.12);
                border-color: rgba(255, 94, 1, 0.55);
                color: #c2410c;
              }
              ...
              .investment-market__primary {
                background: linear-gradient(135deg, #ff5e01, #ff7a2f);
                color: #ffffff;
                box-shadow: 0 16px 35px -20px rgba(255, 94, 1, 0.75);
              }

              .investment-market__primary:hover {
                transform: translateY(-2px);
                box-shadow: 0 20px 45px -20px rgba(255, 94, 1, 0.8);
              }
              ...
              @media (max-width: 1024px) {
                ...
                .investment-market__card {
                  flex: 0 0 clamp(320px, 55vw, 420px);
                  box-shadow: none;
                }
              }
          ...
          <section className="section-cta">
            <div className="padding-global" style={{ maxWidth: '1600px', margin: '0 auto', padding: '0 48px' }}>
                ...
                <div className="padding-section-medium">
                  <div data-w-id="03b4adc1-f918-bae5-37d1-18bef1a11870" className="cta-component">
                    <div className="cta-content">
                      <h2 className="heading-style-h2">Join the future of real estate</h2>
                      <div className="cta-button-wrapper">
                    ...
                    <div className="cta-image-wrapper">
                      <img src="https://cdn.prod.website-files.com/68a06250db2face4039500cc/68a70f8af95c3cf69a01112e_Cta%20Image.png" 
                           loading="lazy" 
                    ...
                    <div className="cta-glass-image-wrapper">
                      <img src="https://cdn.prod.website-files.com/68a06250db2face4039500cc/68a33e286e2e1212a5ec3dd9_glass.png" 
                           loading="lazy" 
                           alt="" 
                  ...
                  <div className="testimonial-component">
                    ...
                    <div data-w-id="7700d1c4-64a2-cbf3-320e-4553985a2ee6" className="testimonial-bottom-content-wrap">
                      <div data-delay="4000" 
                        ...
                        <div className="testimonial-slider-mask w-slider-mask">
                            ...
                            <div className="testimonial-card">
                              <div className="testimonial-top-content-wrapper">
                                ...
                                <div className="testimonial-right-star-wrap">
                                  {[...Array(5)].map((_, i) => (
                                    <img key={i} src="https://cdn.prod.website-files.com/68a06250db2face4039500cc/68aae546cc071081e444df28_Star%2011.svg" loading="lazy" alt="" className="star-image" />
                                  ))}
                                </div>
                              </div>
                              <h6 className="heading-style-h6 is-testimonial">
                                "I've tried several property management tools, but none compare to this. The dashboards are clear, automation features are fantastic, and everything feels seamless. Rent collection, tenant communication, and lease management are now stress-free. I highly recommend it to anyone managing multiple units. It has truly elevated the way I run my properties."
                              </h6>
                          ...
                          <div className="testimonial-slide w-slide">
                            <div className="testimonial-card">
                              ...
                              <h6 className="heading-style-h6 is-testimonial">
                                "Managing multiple properties used to be chaotic, but this platform changed everything. I can track payments, schedule maintenance, and communicate with tenants all in one place. It's intuitive, reliable, and has saved me countless hours each week. Truly a game-changer for property managers! I now feel more in control and confident about my property business than ever before."
                              </h6>
                              <div className="testimonial-bottom-content">
                                <div className="testimonial-bottom-left-content">
                                  <div className="testimonial-author-image-wrap">
                        ...
                        <div className="testimonial-left-arrow w-slider-arrow-left">
                        ...
                        <div className="testimonial-right-arrow w-slider-arrow-right">
                          <div className="testimonial-arrow-wrap">
                        ...
                        <div className="testimonial-nav w-slider-nav w-slider-nav-invert w-round"></div>
                      </div>
                    </div>
                  </div>
                </div>
